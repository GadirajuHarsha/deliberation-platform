...
export default function Auth() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (appMode === 'demo') {
      ...
      const result = await login('guest@demo.local', 'password');
    ...
    <div className="max-w-md mx-auto mt-16 bg-white p-8 border border-surface-200 shadow-sm rounded-xl">
      ...
      {appMode === 'demo' ? (
        <div className="space-y-6">
          <button
            onClick={() => {
              console.log("[Auth] Demo entry clicked");
              handleSubmit({ preventDefault: () => {} });
            }}
            disabled={loading}
            className="w-full bg-primary-600 hover:bg-primary-700 text-white font-black py-5 rounded-2xl shadow-xl shadow-primary-900/20 transition-all flex items-center justify-center text-lg active:scale-95"
          >
            {loading ? <Loader2 className="h-6 w-6 animate-spin" /> : 'Enter Dashboard'}
          </button>
          
          <div className="bg-surface-50 p-4 rounded-xl border border-surface-200">
             <p className="text-[10px] text-surface-400 font-bold uppercase tracking-widest text-center">
               Restricted Guest Environment
             </p>
          </div>
        </div>
      ) : (
